...
def process_paf_alignments(data_df):
    ...
    data_df = data_df[data_df['Alignment block length'].astype(float) / data_df['Target sequence length'].astype(float) > 0.50]
    ...
    for query_name, group in data_df.groupby("Query sequence name"):
        ...
        i = 0
        while i < len(group):
            # start with a 28S in the plus strand
            if '_28S' in group.at[i, 'Target sequence name'] and group.at[i, 'Relative strand'] == "+":
                #check if the next one is a valid 18S
                if i + 1 < len(group) and '_18S' in group.at[i + 1, 'Target sequence name'] and group.at[i, 'Relative strand'] == group.at[i + 1, 'Relative strand']:
                    # check if the next one is a valid 28S
                    if i + 2 < len(group) and '_28S' in group.at[i + 2, 'Target sequence name'] and group.at[i + 1, 'Relative strand'] == group.at[i + 2, 'Relative strand']:
                        # check if 28S alignment is not on the edge of the contig and that there is 100bp available after the i+2 28S
                        if group.at[i + 2, 'Query end'] + 100 < group.at[i + 2, 'Query sequence length']:
                            morph = group.at[i, 'Query sequence name'], group.at[i, 'Query end'] + 100, group.at[i + 2, 'Query end'] + 100, '+'
                            morphs.add(morph)
                            # morph = group.at[i, 'Query sequence name'] + "\t" + str(group.at[i, 'Query end'] + 100) + "\t" + str(group.at[i + 2, 'Query end'] + 100) + "\t+"
                            # morphs.add(morph)
            # start with a 28S in the minus strand
            elif '_28S' in group.at[i, 'Target sequence name'] and group.at[i, 'Relative strand'] == "-":
                # check if 28S alignment is not on the edge of the contig and that there is 100bp available before the query start
                if group.at[i, 'Query start'] - 100 > 0:
                    #check if the next one is a valid 18S
                    if i + 1 < len(group) and '_18S' in group.at[i + 1, 'Target sequence name'] and group.at[i, 'Relative strand'] == group.at[i + 1, 'Relative strand']:
                        # check if the next one is a valid 28S
                        if i + 2 < len(group) and '_28S' in group.at[i + 2, 'Target sequence name'] and group.at[i + 1, 'Relative strand'] == group.at[i + 2, 'Relative strand']:
                            ...
                            morphs.add(morph)
                            # morph = group.at[i, 'Query sequence name'] + "\t" + str(group.at[i, 'Query start'] - 100) + "\t" + str(group.at[i + 2, 'Query start'] - 100) + "\t-"
                            # morphs.add(morph)
...
def extract_sequences(fasta_file, morphs_df, output_dir):
    ...
    for _, row in morphs_df.iterrows():
        ...
        if seq_id in sequences:
            ...
            extracted_sequences.append((seq_id + ':' + str(start) + '-' + str(end), str(sequence)))

    with open(os.path.join(output_dir, "rDNA.morphs.fasta"), 'w') as output_handle:
...
def main():
    ...
    extract_sequences(args.fasta, morphs_df, args.output_dir)
