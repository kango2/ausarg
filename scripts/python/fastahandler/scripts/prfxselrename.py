...
# Example command: python3 prexselrename.py --input-seq test.fa --out test_out.fa --input-id new_id.txt (w/ optional for --t cpu and --mem memory)
...
def process_fasta(content, id_map):
    ...
    for line in content.split('\n'):
        ...
        if header_match:
            # Only process sequence if current header is in id_map
            if current_header in id_map:
                sequences.append((id_map[current_header], ''.join(current_sequence)))
...
# Main function
def main():
    parser = argparse.ArgumentParser(description='prfxselrename - FASTA file selective processing and renaming tool')
    ...
    if not (args.input_seq.endswith(('.fasta', '.fa', '.fas', '.fna', '.tar.gz', '.gz', '.zip', '.bz2'))):
        raise ValueError("Not a proper file format. Please provide a readable file and sequence format to call the prfxselrename module.")
