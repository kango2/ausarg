...
if [ -z "$seqt" ]; then
    printf "%s\n" "${filelist[@]}" | parallel -j4 -I{} --jobs "$num_jobs" /g/data/xl04/ka6418/TRASH/TRASH_run.sh --par 4 {} --o "${PBS_JOBFS}" --N.max.div 5
else
    printf "%s\n" "${filelist[@]}" | parallel -j4 -I{} --jobs "$num_jobs" /g/data/xl04/ka6418/TRASH/TRASH_run.sh --par 4 {} --o "${PBS_JOBFS}" --N.max.div 5 --seqt "$seqt"
fi
