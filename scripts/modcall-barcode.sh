...
module load /g/data/if89/apps/modulefiles/buttery-eel/0.5.1+dorado7.4.12 samtools htslib
...
/usr/bin/time -v  buttery-eel -i ${MERGED_SLOW5} -o ${BASECALL_OUT}/${sampleid}.sam -g ${ONT_DORADO_PATH} --port ${PORT} --use_tcp --config ${MODEL} -x cuda:all --slow5_threads 10 --slow5_batchsize 4000 --procs 20 --call_mods --trim_adapters -q 7 || die "basecalling failed"

samtools fastq -@ ${PBS_NCPUS} ${BASECALL_OUT}/${sampleid}.pass.sam > ${BASECALL_OUT}/${sampleid}.pass.fastq  || die "samtools fastq failed"
bgzip -@ ${PBS_NCPUS} ${BASECALL_OUT}/${sampleid}.pass.fastq
samtools fastq -@ ${PBS_NCPUS} ${BASECALL_OUT}/${sampleid}.fail.sam > ${BASECALL_OUT}/${sampleid}.fail.fastq  || die "samtools fastq failed"
bgzip -@ ${PBS_NCPUS} ${BASECALL_OUT}/${sampleid}.fail.fastq

samtools view -@ ${PBS_NCPUS} -b ${BASECALL_OUT}/${sampleid}.pass.sam | samtools sort -@ ${PBS_NCPUS} --write-index -o "${BASECALL_OUT}/${sampleid}.pass.bam"##idx##"${BASECALL_OUT}/${sampleid}.pass.bam.bai" || die "samtools convert & sort failed"
samtools view -@ ${PBS_NCPUS} -b ${BASECALL_OUT}/${sampleid}.fail.sam | samtools sort -@ ${PBS_NCPUS} --write-index -o "${BASECALL_OUT}/${sampleid}.fail.bam"##idx##"${BASECALL_OUT}/${sampleid}.fail.bam.bai" || die "samtools convert & sort failed"


rm ${BASECALL_OUT}/${sampleid}.pass.sam 
rm ${BASECALL_OUT}/${sampleid}.fail.sam
