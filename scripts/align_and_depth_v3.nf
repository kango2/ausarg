...
process align_ont {
    ...
    minimap2 -t \${PBS_NCPUS} -ax map-ont ${ref} ${reads} > ${outputdir}/"\${output}.sam"
    samtools view -buS -@ \${PBS_NCPUS} ${outputdir}/"\${output}.sam" | samtools sort -@ \${PBS_NCPUS} - -o ${outputdir}/"\${output}.sorted.ont.bam" && samtools index ${outputdir}/"\${output}.sorted.ont.bam"
...
process align_pb {
    ...
    output=\$(basename "${ref}" .fasta)_\$(basename "${reads}" .fastq.gz)
    minimap2 -t \${PBS_NCPUS} -ax map-pb ${ref} ${reads} > ${outputdir}/"\${output}.sam"
    samtools view -buS -@ \${PBS_NCPUS} ${outputdir}/"\${output}.sam" | samtools sort -@ \${PBS_NCPUS} - -o ${outputdir}/"\${output}.sorted.pb.bam" && samtools index ${outputdir}/"\${output}.sorted.pb.bam"
