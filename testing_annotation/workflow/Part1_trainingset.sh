...
#PBS -l ncpus=48,walltime=16:00:00,storage=gdata/if89+gdata/xl04,mem=100GB
#PBS -j oe
...
module load blast/2.11.0 Augustus/3.4.0 perllib/v5.26.3 blat/37 RepeatMasker/4.1.2-p1 scipio/1.4 pblat/2.5 pslCDnaFilter/0 parallel/20191022 genometools/1.6.2 seqtk/1.3 exonerate/2.2.0
...
cp ${trinity_out} .
fastasplit -f ${trinity_out} -o . -c 10
...
cd ${workingdir}/Simple_header/trinity_all
export trinity_out_BASE=$(basename ${trinity_out})
for i in $(ls -1v *chunk_00* | sed "s/${trinity_out_BASE}_chunk_//g"); do echo $i; done \
| parallel --jobs 10 \
blastx -query ${workingdir}/Simple_header/trinity_all/${trinity_out_BASE}_chunk_{} \
-db ${workingdir}/Trinity_filter/first_filter/database/uniprot_sprot \
-parse_deflines \
-outfmt 6 \
-max_target_seqs 1 \
-num_threads ${PBS_NCPUS} \
-out ${workingdir}/Trinity_filter/first_filter/blast/output_{}.tsv

# translating output
cd ${workingdir}/Trinity_filter/first_filter/blast
for i in $(ls -1v output_*); do cat $i >> output.tsv; done
