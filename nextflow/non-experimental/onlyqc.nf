...
process longread_qc {
    ...
    project = 'xl04'
    time = '1h'
    clusterOptions = '-l ncpus=1,mem=64GB,storage=gdata/if89+gdata/xl04'
...
workflow {
    ...
    pacbio_fastqs = channel
        .fromQuery('select blurb, flowcell, platform, filename from SRA where platform is "PACBIO_SMRT" and blurb is "BASDU"', db: 'inputdb')
        .map { row ->
            def (title, flowcell, platform, filename) = row
            def output = "${params.topfolder}" 
            return [filename, title, flowcell, output, platform]
        }.view()

    longread_qc(pacbio_fastqs)

    ont_fastqs = channel
    .fromQuery('select blurb, flowcell, platform, filename from SRA where platform is "OXFORD_NANOPORE" and blurb is "BASDU"', db: 'inputdb')
    .map { row ->
        def (title, flowcell, platform, filename) = row
        def output = "${params.topfolder}"
        return [filename, title, flowcell, output, platform]
