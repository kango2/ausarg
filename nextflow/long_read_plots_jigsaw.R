...
if (pacbio_only || (!pacbio_only && !ont_only)) {
    ...
    file_paths <- list.files(path = top_folder, pattern = "_PACBIO_SMRT_length_freq\\.csv$", full.names = TRUE)

    # Extract dataset names from file paths
    dataset_names <- str_replace(basename(file_paths), "_PACBIO_SMRT_length_freq\\.csv$", "")
    ...
    file_paths <- list.files(path = top_folder, pattern = "PACBIO_SMRT_quality_freq", full.names = TRUE)
    ...
    pdf_file_name <- paste(sample_name, collapse = "_")
    pdf_file_name <- paste0(pdf_file_name, "_PACBIO_SMRT_quality_freq.pdf")
...
if (ont_only || (!pacbio_only && !ont_only)) {
        ...
        # Detect all files with _ont_length_freq in their names within the top folder
    file_paths <- list.files(path = top_folder, pattern = "_OXFORD_NANOPORE_length_freq\\.csv$", full.names = TRUE)

    # Extract dataset names from file paths
    dataset_names <- str_replace(basename(file_paths), "_OXFORD_NANOPORE_length_freq\\.csv$", "")
    ...
    sample_name <- str_extract(dataset_names[1], "^[^_]+")
    pdf_file_name <- paste(sample_name, collapse = "_")
    pdf_file_name <- paste0(pdf_file_name, "_OXFORD_NANOPORE_length_freq.pdf")
    ...
    ggsave(filename = pdf_full_path, plot = plot, device = "pdf", width = 11, height = 8.5)
        ...
        # List all CSV files in the top folder that contain "pacbio_quality_freq"
    file_paths <- list.files(path = top_folder, pattern = "_OXFORD_NANOPORE_quality_freq", full.names = TRUE)
    ...
    data <- read.csv(file_path)
    file_name <- basename(file_path)
    legend <- gsub("_OXFORD_NANOPORE_quality_freq.*", "", file_name)  # Extract date part as legend
    data$Legend <- legend  # Add a 'Legend' column with the extracted date
    dfs[[length(dfs) + 1]] <- data
    }
    ...
    pdf_file_name <- paste(sample_name, collapse = "_")
    pdf_file_name <- paste0(pdf_file_name, "_OXFORD_NANOPORE_quality_freq.pdf")
